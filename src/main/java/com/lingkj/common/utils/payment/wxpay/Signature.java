...
//        Util.log("恭喜，API返回的数据签名验证通过!!!");
     ...
     * @throws SAXException
     */
    public static boolean checkIsSignValidFromResponseMap(Map<String, Object> map, String key) throws ParserConfigurationException, IOException, SAXException {

//       Util.log(map.toString());
        System.out.println("签名验证开始");
        String signFromAPIResponse = map.get("sign").toString();
        if (signFromAPIResponse == "" || signFromAPIResponse == null) {
//           Util.log("API返回的数据签名数据不存在，有可能被第三方篡改!!!");
            return false;
